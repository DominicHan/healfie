package com.fn.healfie.component.bottombar;
...
public class TopBar extends View {
    ...
    public TopBar setContainer(int containerId) {
        this.containerId = containerId;
        return this;
    }

    public TopBar setTitleBeforeAndAfterColor(String beforeResCode, String AfterResCode) {//支持"#333333"这种形式
        titleColorBefore = Color.parseColor(beforeResCode);
        titleColorAfter = Color.parseColor(AfterResCode);
        return this;
    }

    public TopBar setTitleSize(int titleSizeInDp) {
        this.titleSizeInDp = titleSizeInDp;
        return this;
    }

    public TopBar setIconWidth(int iconWidth) {
        this.iconWidth = iconWidth;
        return this;
    }

    public TopBar setTitleIconMargin(int titleIconMargin) {
        this.titleIconMargin = titleIconMargin;
        return this;
    }

    public TopBar setIconHeight(int iconHeight) {
        this.iconHeight = iconHeight;
        return this;
    }

    public TopBar addItem(Class fragmentClass, String title, int iconResBefore, int iconResAfter) {
    ...
    public TopBar setFirstChecked(int firstCheckedIndex) {//从0开始
    ...
    private int[] titleBaseLine = new int[3];
    ...
    private void initParam() {
        int[] widths = {26,20,19};
        int[] heights = {24,22,23};
        if (itemCount != 0) {
            //单个item宽高
            parentItemWidth = getWidth() / itemCount;
            Log.e("111111111111111", "initParam: "+getWidth());
            Log.e("111111111111111", "initParam: "+itemCount);
            Log.e("111111111111111", "initParam: "+parentItemWidth);
            int parentItemHeight = getHeight();
            //图标边长
            Rect rect = new Rect();

            for (int i = 0; i < itemCount; i++) {
                int iconWidth = dp2px(widths[i]);//先指定20dp
                int iconHeight = dp2px(heights[i]);
                ...
                titleBaseLine[i]= parentItemHeight - iconTop;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (itemCount != 0) {
            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                canvas.drawText(title, x, titleBaseLine[i], paint);
            }
        }
    }
